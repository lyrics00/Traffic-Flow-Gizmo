...
def run_game(screen, bg, img_x, img_y, clock):
    ...
    road1 = Road(num_lanes=3, lane_height=100, lane_width=130, road_x_position=410, road_y_position=-250, lane_directions=["straight", "straight", "straight"], orientation="down", restriction_time= 1000.0)
    road2 = Road(num_lanes=1, lane_height=100, lane_width=130, road_x_position=1050, road_y_position=220, lane_directions=["straight"], orientation="left", restriction_time= 2000.0)
    road3 = Road(num_lanes=3, lane_height=100, lane_width=100, road_x_position=440, road_y_position=800, lane_directions=["straight", "straight", "straight"], orientation="up", restriction_time= 1000.0)
    road4 = Road(num_lanes=1, lane_height=100, lane_width=100, road_x_position=-300, road_y_position=160, lane_directions=["straight"], orientation="right", restriction_time= 2000.0)
...
def GenerateCars(list: list[Car], intersection: Intersection):
    ...
    if num < 100:
        ...
        lanes = rand_road.getLanes()
        #if lanes[lane_num].TimeDone():
        list.append(CreateCar(image, SPEED, intersection, rand_road, lane_num))
    return list
